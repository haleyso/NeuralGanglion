...
class ImageEventRestorationModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        if self.debug: 
            print("voxel ",self.voxel.size(), self.voxel.min(), self.voxel.max(), self.voxel.mean(), self.voxel.std() )

        if 'gt' in data:
            self.gt = data['gt'].to(self.device)
        # print("self.gt ",self.gt.size())
        # sys.exit()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img and idx%100==0:
                tb_logger.add_image('gt/pred', make_grid(torch.concat([visuals['gt'].detach(), visuals['result'].detach()], dim=3).cpu(), nrow=8, normalize=True), current_iter+idx)
