...
class BaseModel():
    ...
    def validation(self, dataloader, current_iter, tb_logger, save_img=False, rgb2bgr=True, use_image=True, save_all_imgs=False):
        """Validation function.
        ...
        if self.opt['dist']:
            ...
            return self.nondist_validation(dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image, save_all_imgs)
    ...
    def load_network(self, net, load_path, strict=True, param_key='params'):
        ...
        for k, v in deepcopy(load_net).items():
            if k.startswith('module.'):
                load_net[k[7:]] = v
                load_net.pop(k)
            if k in ['pos_thres_nominal', 'neg_thres_nominal', 'p_pos', 'p_neg']:
                if v.shape != net.state_dict()[k].shape:
                    if v.dim() == 0:
                        print(f'{k} is 0 dim, reshape to {net.state_dict()[k].shape}')
                        v = v.unsqueeze(0)
                    load_net[k] = v.reshape(net.state_dict()[k].shape)
        self._print_different_keys_loading(net, load_net, strict)
        net.load_state_dict(load_net, strict=strict)
        # print(net.state_dict().keys(), "loaded")
        # sys.exit()
