...
from basicsr.data.event_util import make_event_preview
...
class TwoSharpImageEventRecurrentRestorationModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        # print(self.lq.size(), "LQ SIZE") #torch.Size([8, 2, 3, 256, 256]) LQ SIZE
        # sys.exit()
        ...
        # print()
        # print(data['seq'][0], self.lq.min(), self.lq.max(), self.lq.mean(),self.lq.std() )
        # print("EVENTS: ", self.voxel.min(), self.voxel.max(), self.voxel.mean())
        # sys.exit()
    ...
    def optimize_parameters(self, current_iter):
        ...
        # print(self.lq.size(), self.voxel.size())
        # torch.Size([8, 2, 3, 256, 256]) torch.Size([8, 7, 2, 256, 256])
        # sys.exit()
        bin_minus = self.voxel[:,:,0,:,:]
        bin_plus = self.voxel[:,:,1,:,:]
        # print("bin_minus ", bin_minus.min().cpu().numpy(),bin_minus.max().cpu().numpy(),bin_minus.mean().cpu().numpy(),bin_minus.std().cpu().numpy())
        # print("bin_plus ", bin_plus.min().cpu().numpy(),bin_plus.max().cpu().numpy(),bin_plus.mean().cpu().numpy(),bin_plus.std().cpu().numpy())
        # print(self.voxel.min().cpu().numpy(),self.voxel.max().cpu().numpy(),self.voxel.mean().cpu().numpy(),self.voxel.std().cpu().numpy())
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            cnt += 1
            if save_img and idx%100== 0 and tb_logger:
                ...
                first_events =  make_event_preview(visuals['voxel'][0,0,0,:,:].cpu().numpy(), mode='red-blue', num_bins_to_show=-1).transpose(2,0,1)
                tb_logger.add_image('VAL first events', first_events, current_iter+idx)
                last_events =  make_event_preview(visuals['voxel'][0,0,0,:,:].cpu().numpy(), mode='red-blue', num_bins_to_show=-1).transpose(2,0,1)
