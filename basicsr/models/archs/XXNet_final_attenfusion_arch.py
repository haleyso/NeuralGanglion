...
class FinalBidirectionAttenfusion(FinalDecoderRecurrentUNet):
    ...
    def forward(self, x, event):
        ...
        :param event: b, t, num_bins, h, w -> b*t num_bins(2) h w  ---------> b, t, num_event_kinds, num_bins, h, w -> b*t num_event_kinds*num_bins h w  for multi-kinds of events
        ...
        # x.size        torch.Size([2, 2, 3, 256, 256]) 0
        # event.size    torch.Size([2, 7, 2, 256, 256]) 0

        # reshape
        if x.dim()==5:
            x = rearrange(x, 'b t c h w -> b (t c) h w') # sharp
        # b, t, num_bins, h, w = event.size() 
        b, t, num_kinds_events, num_bins, h, w = event.size()
        event = rearrange(event, 'b t nke nb h w -> (b t) (nke nb) h w')

        
        # head
        # torch.cuda.empty_cache()
        # print_stats(x, "x before")
        # print_stats(self.head_img.conv2d.weight, "head_img.conv2d.")
        x = self.head_img(x) # image feat
        head = x
        # print_stats(self.head.conv2d.weight, "head.conv2d.")
